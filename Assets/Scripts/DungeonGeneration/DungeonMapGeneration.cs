...
public class DungeonMapGeneration : MonoBehaviour
{
    ...
    IEnumerator BoardSetup()
    {
        ...
        //óäàëåíèå äî÷åðíèõ ýëåìåíòîâ ïðîøëîé ãåíåðàöèè
        ...
        //óñêîðåíèå îáðàáîòêè ñòîëêíîâåíèé
        Time.timeScale = 10.0f;
        yield return new WaitForSeconds(60); // ïàóçà äëÿ ïðîñ÷èòûâàíèÿ ïîëîæåíèÿ êîìíàò
        Time.timeScale = 1.0f; 
        ...
        for (int i = 0; i < triangulation.pointsOfTriangulationVectors.Count; i++)
        {
            ...
            if (line)
            {
                if (Random.Range(0, 4) == 2) // øàíñ 25% âåðíóòü ðåáðî
                {
                    //âèçóàëèçàöèÿ ðåáåð ãðàôà
        ...
        {
            var xRoom1 = (float)Math.Round(mainRoomsCoords[pointsOfMainCorridors[i].Item1].x); // êîîðäèíàòû öåíòðîâ òî÷åê îòðåçêà
            ...
            var xScaleRoom1 = mainRoomsScales[pointsOfMainCorridors[i].Item1].Item1; // ðàçìåðû êîìíàò äëÿ ýòèõ öåíòðîâ
            ...
            var xMiddle = (float)Math.Round((xRoom1 + xRoom2) / 2); //ñåðåäèíà ìåæäó öåíòðàìè òî÷åê
            var yMiddle = (float)Math.Round((yRoom1 + yRoom2) / 2);

            var xLeftBoundRoom1 = xRoom1 - xScaleRoom1 / 2 + 1; // â êîíöå ïðèáàâëÿåòñÿ/îòíèìàåòñÿ òîëùèíà ñòåí
        ...
        foreach (Transform corridor in corridorHolder.GetComponentsInChildren<Transform>())
        {
            ...
            {
                //çäåñü íå ìåéí êîìíàòû, à âñå îñòàëüíûå
                for (int i = 0; i < numOfRooms; i++)
                {
                    ...
                    if (sideRoom)
                    {
                        var xRoom = sideRoom.transform.position.x; // êîîðäèíàòû öåíòðîâ òî÷åê îòðåçêà
                        var yRoom = sideRoom.transform.position.y;
                        var xScaleRoom = sideRoom.transform.localScale.x; // ðàçìåðû êîìíàò äëÿ ýòèõ öåíòðîâ
        ...
        //Time.timeScale = 10.0f;
        yield return new WaitForSeconds(0.1f); // ïàóçà äëÿ ïðîñ÷èòûâàíèÿ ïîëîæåíèÿ êîìíàò
        //Time.timeScale = 1.0f;
